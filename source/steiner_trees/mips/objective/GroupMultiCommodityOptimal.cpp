...
void GroupMultiCommodityOptimal::create_constraints(mip::MIPModel& mip_model)
{
	...
	for (graph::Net const& net : _group_edges.nets()) {
		for (graph::Edge const& edge : _group_edges.graph().edges()) {
			constraint.add_variable(_group_edges.variables().get(edge.id(), net.name()), edge.weight());
		}

		graph::Node const& root_node = _group_edges.graph().node(net.terminal(0));
