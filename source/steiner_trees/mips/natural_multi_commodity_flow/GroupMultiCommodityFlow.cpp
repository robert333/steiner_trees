...
//					graph::NodeId const& outgoing_node = outgoing_edge.opposite(node_id);
					...
					constraint->SetCoefficient(_variables.get(outgoing_edge_id, net.name(), terminal_id), 1);
				}
...
//					graph::NodeId const& incoming_node = incoming_edge.opposite(node_id);
					...
					constraint->SetCoefficient(_variables.get(incoming_edge_id, net.name(), terminal_id), -1);
				}
